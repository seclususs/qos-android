...
namespace SystemUtils {
    bool applyTweak(const std::string& path, const std::string& value) {
        ...
        if (!outfile) {
            LOGE("Failed to open for writing: %s (errno: %d - %s)", path.c_str(), errno, strerror(errno));
            return false;
        }
        outfile << value;
        if (outfile.fail()) {
    ...
    void setSystemProp(const std::string& key, const std::string& value) {
        if (__system_property_set(key.c_str(), value.c_str()) < 0) {
            LOGE("Failed to set system property: %s (errno: %d - %s)", key.c_str(), errno, strerror(errno));
        }
    }
    ...
    bool setAndroidSetting(const std::string& property, const std::string& value) {
        ...
        int pipefd[2];
        if (pipe(pipefd) == -1) {
            LOGE("setAndroidSetting: pipe() failed (errno: %d - %s)", errno, strerror(errno));
            return false;
        }
        pid_t pid = fork();
        if (pid == -1) {
            LOGE("setAndroidSetting: fork() failed (errno: %d - %s)", errno, strerror(errno));
            close(pipefd[0]);
            close(pipefd[1]);
            return false;
        }
