...
impl RefreshManager {
    pub fn new() -> Result<Self, QosError> {
        log::info!("RefreshManager: Starting Display Service...");
        ...
        manager.apply_mode(DisplayMode::LowPower);
        Ok(manager)
    }
    fn apply_mode(&mut self, mode: DisplayMode) {
        ...
        log::info!("Display: Switching to {}Hz", val);
    ...
    fn on_timeout(&mut self) {
        if self.current_mode == DisplayMode::Smooth {
            if self.last_interaction.elapsed() >= K_IDLE_TIMEOUT {
                log::debug!("Idle detected -> Dropping to 60Hz");
