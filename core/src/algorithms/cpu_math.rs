...
pub fn calculate_trend_gain(avg10: f64, avg60: f64, memory_psi: f64, tunables: &CpuTunables) -> f64 {
    ...
    let damped_gain = base_gain / (1.0 + memory_penalty);
    damped_gain
}
...
pub fn calculate_migration_cost(delta_smooth: f64, p_eff: f64, memory_psi: f64, tunables: &CpuTunables) -> f64 {
...
pub fn calculate_latency_and_granularity(
    ...
    let adjusted_latency = (raw_latency * memory_time_dilation)
        .clamp(thermal_floor, tunables.max_latency_ns * 1.5);
    let raw_gran = adjusted_latency * tunables.latency_gran_ratio;
    let target_min_gran = raw_gran
        .clamp(tunables.min_granularity_ns, tunables.max_granularity_ns * 1.5);
