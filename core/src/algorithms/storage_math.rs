...
pub fn calculate_read_ahead(p_curr: f64, tunables: &StorageTunables) -> f64 {
    if p_curr < tunables.io_read_ahead_threshold {
        ...
        let result =
            tunables.min_read_ahead + scaling * (tunables.max_read_ahead - tunables.min_read_ahead);
...
pub fn calculate_queue_params(i_sat: f64, tunables: &StorageTunables) -> (f64, f64) {
    ...
    let target_nr =
        (tunables.max_nr_requests * (1.0 - sat_factor)) + (tunables.min_nr_requests * sat_factor);
    let target_fifo =
        (tunables.max_fifo_batch * (1.0 - sat_factor)) + (tunables.min_fifo_batch * sat_factor);
