...
use crate::config::loop_settings::{
    ...
    BOOT_WAIT_RETRY_LIMIT,
    BOOT_POLL_INTERVAL_SEC,
    COOLDOWN_DURATION_SEC
};
use crate::registry::boot_tweaks;
use crate::hal::{filesystem, properties, bridge};
            ...
            Err(e) => {
                match &e {
                    ...
                    QosError::SystemCheckFailed(msg) | QosError::PermissionDenied(msg) => {
                        log::error!("Service '{}' failed: {}. Disabling permanently.", self.name, msg);
                        self.is_permanently_disabled = true;
                    },
                    _ => {
                        log::error!("Failed to initialize service '{}': {}. Retrying...", self.name, e);
...
pub fn apply_system_tweaks() {
    ...
    log::info!("Rust: Applied {}/{} file tweaks.", success_count, file_tweaks.len());
...
pub fn wait_for_boot_completion(tag: &str) {
    ...
    loop {
        ...
        match get_system_property("sys.boot_completed") {
            Ok(val) => if val == "1" { break; },
            Err(_) => {}
        }
        retry_count += 1;
    ...
    if !SHUTDOWN_REQUESTED.load(Ordering::Acquire) {
        log::info!("Rust [{}]: Stabilizing for {}s...", tag, STABILIZATION_DELAY_SEC);
        ...
        for (i, service) in services.iter_mut().enumerate() {
            ...
            if service.handler.is_none() {
                    ...
                    if elapsed >= cooldown_dur {
                        if service.try_initialize() {
                            if let Some(ref h) = service.handler {
                                ...
                                if !epoll_mod(epoll_fd.as_raw_fd(), h.as_raw_fd(), i as u64, 
                                libc::EPOLL_CTL_ADD, flags) {
                        ...
                        if wakeup_time < next_wakeup { next_wakeup = wakeup_time; }
                    }
                }
            }
                ...
                if interval_ms > 0 {
                    ...
                    if deadline < next_wakeup { next_wakeup = deadline; }
                }
            }
        }
            ...
            if let Some(service) = services.get_mut(id) {
                if let Some(ref mut handler) = service.handler {
                    match handler.on_event() {
                        Ok(LoopAction::Pause) => service.unregister_if_active(epoll_fd.as_raw_fd(), id as u64),
                        Ok(LoopAction::Resume) => {
                             if !service.registered_in_epoll {
                                 let flags = handler.get_poll_flags();
                                 epoll_mod(epoll_fd.as_raw_fd(), handler.as_raw_fd(), id as u64, 
                                 libc::EPOLL_CTL_ADD, flags);
                                 service.registered_in_epoll = true;
                             }
                        },
                    ...
                    if elapsed >= interval_ms {
                        ...
                        match handler.on_timeout() {
                            Ok(LoopAction::Resume) => {
                                 if !service.registered_in_epoll {
                                    let flags = handler.get_poll_flags();
                                    epoll_mod(epoll_fd.as_raw_fd(), handler.as_raw_fd(), i as u64, 
                                    libc::EPOLL_CTL_ADD, flags);
                                    service.registered_in_epoll = true;
                                }
                            },
